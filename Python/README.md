...
For example, in terminal type in
     ...
     python test_script.py path/to/point_cloud.ply --result --runtime --visualize

For multiquadric

     python multiquadric_test.py path/to/point_cloud.ply --export --visualize
...
#### Visualize recovered superquadrics
##### Show all superquadrics
     python tests/visualize_json.py data/turtle/superquadrics.json

##### Show only specific ones by index
     python tests/visualize_json.py data/turtle/superquadrics.json --index 0 2 5

##### Adjust resolution
     python tests/visualize_json.py data/turtle/superquadrics.json --arcLength 0.1

Each superquadric gets a distinct color, and it prints the shape/scale parameters for reference.